    ...
    <div className="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50">
      <div className="bg-white p-6 rounded-lg shadow-lg max-w-md w-full">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="mb-4">
            <label className="block text-gray-600" htmlFor="date">Date</label>
            <input
              type="text"
              id="date"
              className="w-full p-2 border border-gray-300 rounded-md"
          ...
          <div className="mb-4">
            <label className="block text-gray-600" htmlFor="timeSlot">Time Slot</label>
            <input
              type="text"
              id="timeSlot"
              className="w-full p-2 border border-gray-300 rounded-md"
...
const Calendar = ({ onDateSelect }) => {
  ...
  useEffect(() => {
    axios.get("http://localhost:5000/timeslot/slots")
      .then((response) => {
  ...
  const openBookingModal = (slot) => {
    setSelectedSlot({
      ...slot,
      date: dayjs(slot.date).format("YYYY-MM-DD"), // Ensure proper formatting
    });
    setIsModalOpen(true);
  };
  ...
  return (
    <div className="flex flex-col items-center bg-white shadow-lg rounded-2xl p-6 max-w-lg h-auto mx-auto">
